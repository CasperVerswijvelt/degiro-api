...
import { debug, fetch, processGetCashFundsResultListObject } from "../utils";
...
export function getCashFundstRequest({
  ...
  return new Promise((resolve, reject) => {
    ...
    const uri = `${accountConfig!.data.tradingUrl}${GET_GENERIC_DATA_PATH}${
    ...
    fetch(uri, requestOptions, userAgent)
      ...
      .then((res) => {
        if (
          !res.cashFunds ||
          !res.cashFunds.value ||
          !Array.isArray(res.cashFunds.value)
        ) {
