...
export function getAccountConfigRequest(
  { jsessionId, userAgent }: DeGiro,
  sessionId?: string
): Promise<AccountConfigType> {
  return new Promise((resolve, reject) => {
    ...
    debug(
      `Making request to ${BASE_API_URL}${GET_ACCOUNT_CONFIG_PATH} with JSESSIONID: ${
        sessionId || jsessionId
      }`
    );
    fetch(BASE_API_URL + GET_ACCOUNT_CONFIG_PATH, requestOptions, userAgent)
      .then((res) => {
        if (!res.ok) {
