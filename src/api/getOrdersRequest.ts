...
export function getOrdersRequest(
  ...
  config: GetOrdersConfigType
): Promise<GetOrdersResultType> {
  return new Promise((resolve, reject) => {
    ...
    const uri = `${accountConfig!.data.tradingUrl}v5/update/${
    ...
    fetch(uri, requestOptions, userAgent)
      ...
      .then((res) => {
        const result: GetOrdersResultType = {
          orders: res.orders
            ? res.orders.value.map(processGetOrdersResultListObject)
            : [],
          lastTransactions: res.transactions
            ? res.transactions.value.map(processGetOrdersResultListObject)
            : [],
        };
        resolve(result);
      })
