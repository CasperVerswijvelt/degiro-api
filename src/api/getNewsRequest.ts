...
export function getNewsRequest(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const latestNewsURI = `${
      accountConfig!.data.companiesServiceUrl
    }${GET_LATESTS_NEWS_PATH}?${params}`;
    const topNewsURI = `${
      accountConfig!.data.companiesServiceUrl
    }${GET_TOP_NEWS_PATH}?${params}`;
    ...
    try {
      ...
      if (top) {
        const latestFetch = await fetch(topNewsURI, requestOptions, userAgent);
        const { data } = await latestFetch.json();
        result.top = data;
      }
    } catch (error) {
